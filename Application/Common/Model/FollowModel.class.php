...
class FollowModel extends Model {
    ...
    function init_follow_by_mobile (  $openid, $token = '', $mobile, $has_subscribe = false) {
        empty ( $token ) && $token = get_token ();
        ...
        $umap ['openid'] = $data ['openid'] = $openid;
        $datas = $data;
        $flower = M('apps_follow');
        $user = $flower->where ( $umap )->find();

        $uid  = isset($user['uid'])? $user['uid'] : 0;

        if ($uid > 0) {
        	$user['mobile'] = $mobile;
            $flower->where ( $umap )->save($user);
            /*dump($flower->getLastSql());
            dump($user);*/
